...
namespace
{
	std::pair<::sockaddr_un, std::size_t> make_local_sockaddr(const std::string& name)
	{
		if (name.empty())
		...
		if (name[0] == '/')
		{
			std::memcpy(sockaddr.sun_path, name.data(), name.size());
			return {sockaddr, offsetof(sockaddr_un, sun_path) + name.size() + 1};
		}
#ifdef __linux__
		else if (name[0] == '@')
		{
			std::memcpy(sockaddr.sun_path + 1, name.data() + 1, name.size() - 1);
			return {sockaddr, offsetof(sockaddr_un, sun_path) + name.size()};
		}
		else
		{
			std::memcpy(sockaddr.sun_path + 1, name.data(), name.size());
			return {sockaddr, offsetof(sockaddr_un, sun_path) + name.size() + 1};
		}
#else
		else
...
namespace ynet
{
	...
	const std::size_t LocalBufferSize = 64 * 1024;
